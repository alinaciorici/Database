...
CREATE VIEW [sid].[vw_Account_Transformed]
...
SELECT  [LoadId], 
				  ...
				  */
				  ...
				  ([FlexEmployeeUnlimited] > 0 OR [FlexEmployeeUnlimited] > 0) 
						...
						THEN 'Error: Contacts in  Capped/Flex & Unlimited plans'
				...
				WHEN 
				  ...
				  ISNULL(RelationshipId_d.bu_RelationshipId, RelationshipId_CP.bu_RelationshipId)bu_RelationshipId,
                --  [DependantsCovered], 
                 -- [NewContract], 
                --  [AccountManagerName], 
                ...
                --  [Intermediary], 
                  ...
                  --[OverallTotalLives] bu_org_approx_number_of_core_lives, 
...
bu_Openingpopulationfordependants,
				  ...
				  Isnull(Unlimited.bu_org_capped_unlimited_appointments,Capped.bu_org_capped_unlimited_appointments)bu_org_capped_unlimited_appointments,
                  --[AXAWorkingBody], 
				  ...
				  /*
				  -- Validations
				  -- check if account has members in capped and unlimited plans. Members can only be in capped or unlimited plans. 
				  */
				    ...
				    ([CompanyFundedDependantCapped] is null or [CompanyFundedDependantCapped] = 0) AND
					  ([CompanyFundedEmployeeUnlimited] is null or [CompanyFundedEmployeeUnlimited] = 0) AND
					    ([CompanyFundedDependantUnlimited] is null or [CompanyFundedDependantUnlimited] = 0) AND 
						  ([FlexEmployeeUnlimited] is null or [FlexEmployeeUnlimited] = 0) AND 
						    ([FlexDependantUnlimited] is null or [FlexDependantUnlimited] = 0)  
							THEN CONCAT ('Error: No Contact in Capped/Unlimited/Flex plans. ', char(13), 'Hint: Members can only be in one plan type.')
				...
				WHEN 
				  ...
				  ([FlexEmployeeUnlimited] > 0 OR [FlexEmployeeUnlimited] > 0) 
						...
						THEN CONCAT ('Error: Contacts in Capped/Flex & Unlimited plans. ', char(13), 'Hint: Members can only be in one plan type.')
				...
				WHEN 
				  ...
				  ([FlexEmployeeUnlimited] > 0 OR [FlexEmployeeUnlimited] > 0) 
						...
						THEN CONCAT ('Error: Contacts in both Capped/Flex plans. ', char(13), 'Hint: Members can only be in one plan type.')

				WHEN 
				  (
				  ([CompanyFundedEmployeeUnlimited] > 0 OR [CompanyFundedDependantUnlimited] > 0) AND 
				  ([FlexEmployeeUnlimited] > 0 OR [FlexEmployeeUnlimited] > 0) 
				  )		THEN CONCAT ('Error: Contacts in both Unlimited/Flex plans. ', char(13), 'Hint: Members can only be in one plan type.')

				  	WHEN 
				  (
				  ([CompanyFundedEmployeeUnlimited] > 0 OR [CompanyFundedDependantUnlimited] > 0) AND 
				  ([CompanyFundedEmployeeCapped] > 0 OR [CompanyFundedDependantCapped] > 0) 
				  )		THEN CONCAT ('Error: Contacts in both Capped/Unlimited plans. ', char(13), 'Hint: Members can only be in one plan type.')
				  WHEN A.AccountManagerEmailAddress IS NOT NULL AND A.AccountManagerEmailAddress NOT LIKE '%@%' THEN CONCAT ('Error: Account Manager Email Address Not valid. ', char(13), 'Hint: Account Manager Email Address has no @ symbol.')
				  WHEN A.CoordinatorEmailAddress IS NOT NULL AND A.CoordinatorEmailAddress NOT LIKE  '%@%' THEN CONCAT ('Error: Coordinator Email Address Not valid. ', char(13), 'Hint: Coordinator Email Address has no @ symbol.')
				  WHEN A.PrimaryCompEmail IS NOT NULL AND A.PrimaryCompEmail NOT LIKE  '%@%' THEN CONCAT ('Error: Primary Comp Email Not valid. ', char(13), 'Hint: Primary Comp Email has no @ symbol.')

				  WHEN ISNULL(groupn.AccountName,groupnp.AccountName) IS NOT NULL THEN CONCAT ('Error: Group Number matches an existing Account. ', char(13), 'Hint: Group Numbers are unique per account.')
				  WHEN groupa.AccountName IS NOT NULL THEN CONCAT ('Error: Account name closely matches an existing Account with a differnt group number. ', char(13), 'Hint: Update accordingly and approve or advice the data team further.')

				  ELSE NULL END AS BusinessRuleCheck
...
(
SELECT TOP 1 s.id bu_axasalestype
...
(
SELECT TOP 1 [DependentsPaidForId] bu_org_dependant_paid_for_already  
...
(
SELECT TOP 1 [RelationshipId] [bu_RelationshipId] 
...
(
SELECT TOP 1 [CappedOrUnlimitedId] bu_org_capped_unlimited_appointments
...
OUTER APPLY 
(
SELECT top 1 ac.AccountId, ac.AccountName FROM [dw].[Account] AC
WHERE A.GroupNo = ac.ExternalAccountReference)
groupn

OUTER APPLY 
(
SELECT top 1  ac.GroupName AccountName FROM [sid].[Account_Processed] AC
WHERE A.GroupNo = ac.GroupNo)
groupnp
...
SELECT top 1 ac.AccountId, ac.AccountName FROM [dw].[Account] AC
WHERE 
(A.GroupNo <> ac.ExternalAccountReference)
and ac.ExternalAccountReference is not NULL AND A.GroupNo IS NOT null
and 
admin.fn_FuzzyMatchString(a.GroupName, ac.AccountName) > LEN(a.GroupName) / 1.9
)
groupa
